...
router.put('/', async (req, res) => {
  try {
    ...
    const existingResult = await db.query('SELECT * FROM settings ORDER BY id LIMIT 1');
    ...
    const verifyResult = await db.query('SELECT language FROM settings WHERE id = $1', [updatedSettings.id]);
