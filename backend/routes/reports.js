...
router.get('/products', async (req, res) => {
  try {
    const result = await db.query(
      'SELECT product_id, name, item_name_english FROM products ORDER BY COALESCE(item_name_english, name) ASC'
    );
...
router.get('/customers-due', async (req, res) => {
  try {
    ...
    query += `
      GROUP BY c.customer_id, c.name, c.phone, c.current_balance, c.opening_balance
...
router.get('/dashboard-summary', async (req, res) => {
  try {
    ...
    const purchasesQuery = `
      SELECT 
        COALESCE(SUM(quantity * purchase_price), 0) as total_purchases
      FROM purchases p
      ${purchasesWhere}
    `;
...
router.get('/profit', async (req, res) => {
  try {
    ...
    const purchasesWhere = dateWhere.replace(/date/g, 'p.date');
    const purchasesQuery = `
      SELECT COALESCE(SUM(quantity * purchase_price), 0) as total_purchases
      FROM purchases p
      ${purchasesWhere}
    `;
    ...
    transactions.forEach(trans => {
      if (trans.type === 'purchase') {
        ...
        history.push({
          date: trans.date,
          type: 'purchase',
          description: `Purchase - ${trans.product_name}`,
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const settingsQuery = `SELECT other_app_settings FROM settings ORDER BY id LIMIT 1`;
    ...
    const todayCashSalesQuery = `
      ...
      FROM sales
      WHERE date >= $1 AND date <= $2 AND payment_type = 'cash'
    `;

    // Customer payments received today (cash)
    ...
    // Supplier payments made today (cash)
    ...
    // 5. Supplier Due - Sum of all unpaid supplier balances
    // Use same calculation as /reports/suppliers-payable endpoint for consistency
      ...
      SELECT 
        COALESCE(SUM(
          ...
          COALESCE((
            SELECT SUM(total_amount)
            FROM purchases
            WHERE supplier_id = suppliers.supplier_id AND payment_type = 'credit'
          ), 0) - 
          COALESCE((
            SELECT SUM(amount)
            FROM supplier_payments
