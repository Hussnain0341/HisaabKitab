...
const ConnectionStatus = ({ onRefresh, readOnlyMode, setReadOnlyMode }) => {
  ...
  const checkConnection = async () => {
    ...
    try {
      ...
      if (healthResponse.data && healthResponse.data.status === 'ok') {
        setConnected(true);
        setReadOnlyMode(healthResponse.data.mode === 'read-only' || isClientMode());
      } else {
        setConnected(false);
      }
    } catch (error) {
